...
def is_horizontal_win(pos):
    ...
    index_x = get_column_index(x)
    index_y = get_row_index(y)

    val = game_state[index_y][index_x]
    count = 1

    temp_x_left = index_x - 1

    while temp_x_left >= 0 and game_state[index_y][temp_x_left] == val:
        count += 1
        temp_x_left -= 1

    temp_x_right = index_x + 1

    while temp_x_right <= len(game_state[index_y]) - 1 and game_state[index_y][temp_x_right] == val:
        count += 1
        temp_x_right += 1
...
def is_vertical_win(pos):
    ...
    temp_up = index_y - 1
    while temp_up >= 0 and game_state[temp_up][index_x] == val:
        count += 1
        temp_up -= 1

    temp_down = index_y + 1
    while temp_down <= len(game_state)-1 and game_state[temp_down][index_x] == val:
        count += 1
        temp_down += 1
...
def is_right_to_left_win(pos):
    ...
    temp_x = index_x - 1
    temp_y = index_y - 1
    while temp_x <= 0 and temp_y <= 0 and game_state[temp_y][temp_x] == val:
        count += 1
        temp_x -= 1
        temp_y -= 1
    ...
    temp_x = index_x + 1
    temp_y = index_y + 1
    limit_x = len(game_state[index_y])
    limit_y = len(game_state)
    while temp_x < limit_x and temp_y < limit_y and game_state[temp_y][temp_x] == val:
        count += 1
        temp_x += 1
        temp_y += 1
...
def is_left_to_right_win(pos):
    ...
    temp_y = index_y - 1
    limit_x = len(game_state[index_y])
    while temp_x < limit_x and temp_y >= 0 and game_state[temp_y][temp_x] == val:
        count += 1
        temp_x += 1
        temp_y -= 1
    ...
    temp_y = index_y + 1
    limit_y = len(game_state)
    while temp_x >= 0 and temp_y < limit_y and game_state[temp_y][temp_x] == val:
        count += 1
        temp_x -= 1
        temp_y += 1
